      ...
      <div class="post-article w-full max-w-prose p-5 mx-auto my-0">
        ...
        <div class="post-info">
          ...
          <ul class="post-categories flex flex-wrap justify-end mt-5 -mb-1.5 -mx-1.5">
            <li v-for="(category, index) in sortedCategories" :key="index" class="post-category self-start m-1.5 p-2 text-center text-xs rounded shadow italic filter brightness-90" :style="`background-color: ${blok.background_color.color ? blok.background_color.color : '#e0e0e0'}; color: ${blok.text_color.color};`">
              {{ category }}
            </li>
          </ul>
        </div>
