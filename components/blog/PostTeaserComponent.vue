<template>
  ...
  >
    <NuxtLink :to="postLink" class="teaser-link">
      <div
        :class="`teaser-content h-full flex flex-col ${$parent.blok.row_container && rowContainer.filter(item => item.row_container).length > 1 ? '' : 'lg:flex-row lg:h-64'}`"
      ...
      >
        ...
        />
        <div :class="`teaser-text w-full flex flex-col p-5 ${$parent.blok.row_container && rowContainer.filter(item => item.row_container).length > 1 ? 'lg:h-40 lg:p-10' : 'h-max lg:h-full lg:w-1/2 sm:p-10'} ${$route.name === 'blog' ? 'justify-between' : 'justify-center'}`" :style="`background-color: ${postContent.teaser_background_color.color}; color: ${postContent.teaser_text_color.color};`">
          <div class="text-description">
            <span
              :class="`teaser-title mb-2 font-semibold overflow-hidden ${$route.name === 'blog' ? 'text-xl sm:text-2xl' : 'text-xl'}`"
              :style="`-webkit-line-clamp: ${$parent.blok.row_container && rowContainer.filter(item => item.row_container).length > 1 ? '1' : '2'};`"
            >
              {{ postContent.title }}
            </span>
            <span
              class="teaser-intro overflow-hidden"
              :style="`-webkit-line-clamp: ${$parent.blok.row_container && rowContainer.filter(item => item.row_container).length > 1 ? '2' : '3'};`"
            >
              {{ postContent.intro }}
            </span>
          </div>
