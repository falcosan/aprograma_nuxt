<template>
  <li
    v-if="postContent"
    class="post-teaser overflow-hidden rounded transform transition-all duration-200 hover:shadow hover:scale-105"
  >
    <NuxtLink :to="postLink" class="teaser-link">
      ...
      >
        <div v-show="!wait" :class="`teaser-file w-full ${rowContainer && !carouselContainer && !sliderContainer && !containerContainer ? 'h-44 xs:h-56 sm:h-72 lg:h-64 xl:h-80' : sliderContainer || containerContainer || carouselContainer ? 'h-full' : 'h-44 xs:h-56 sm:h-72 md:h-80 lg:h-full lg:w-1/2'} ${postContent.file.filename ? '' : 'p-5 bg-black'}`">
          <component
            :is="postContent.file.filename ? lookFile() : 'img'"
            :class="`w-full h-full object-cover object-center select-none ${postContent.file.filename ? '' : 'ml-2.5'}`"
        ...
        <Skeleton :class="`w-full ${rowContainer && !carouselContainer && !sliderContainer && !containerContainer ? 'h-44 xs:h-56 sm:h-72 lg:h-64 xl:h-80' : sliderContainer || containerContainer || carouselContainer ? 'h-full' : 'h-44 xs:h-56 sm:h-72 md:h-80 lg:h-full lg:w-1/2'}`" :wait="wait" />
