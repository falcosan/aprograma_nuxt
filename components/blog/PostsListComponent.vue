<template>
  <div v-if="sortedPosts.length > 0" class="posts w-full">
    ...
    <div v-if="blok.categories_action && sortedCategories.length > 0" class="post-categories grid relative overflow-hidden">
      ...
      >
        <ul v-if="showFilters" class="categories-list grid grid-cols-fit-tiny row-start-2 row-end-auto auto-cols-fr gap-5 mb-5">
          ...
          >
            <Input :class="`category-input w-full py-3 px-4 text-sm text-left rounded italic font-light truncate transition-all ${searchCategory.includes(filter) ? 'filter grayscale bg-gray-500' : 'bg-transparent'}`" type="button" :text="filter" />
    ...
    <transition-group
      tag="ul"
      :class="`post-list w-full grid gap-5 auto-cols-fr ${blok.row_container || sliderMode || carouselMode || containerMode ? `${maxPosts} auto-rows-fr` : 'md:grid-cols-fill-medium lg:grid-cols-none lg:grid-flow-row lg:auto-rows-fr'}`"
      appear
      enter-active-class="duration-150"
      leave-active-class="duration-150"
