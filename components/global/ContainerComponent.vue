  ...
  >
    ...
    >
      <div
        ...
        :class="`slider-wrapper relative ${sliderMode || containerMode ? 'flex justify-center' : ''} ${!blok.remove_space ? !blok.background_color_container.color && blok.title ? 'p-5' : 'px-5 pb-5': ''}`"
      >
        <Icon
      ...
      <div v-else class="container-box h-full overflow-hidden">
        <div
          :class="`container-components h-full flex flex-wrap rounded -m-2.5 ${!blok.remove_space ? !blok.background_color_container.color && blok.title ? 'p-5' : 'px-5 pb-5': ''}`"
          :style="`min-height: ${blok.height};`"
        >
          <template v-for="component in elements">
            <div
