...
<template>
  ...
  >
    ...
    <transition enter-active-class="duration-200 linear" leave-active-class="duration-200 linear" enter-class="-translate-y-full" leave-to-class="-translate-y-full">
      <div v-if="alert.message" :class="`form-alert fixed w-full max-w-sm xs:max-w-md sm:max-w-lg md:max-w-none left-1/2 top-10 transform -translate-x-1/2 z-30 p-5 opacity-90 text-center text-sm md:text-base transform text-white ${!$device.isDesktop ? '' : 'md:left-0 md:translate-x-0 md:top-0 md:absolute'} ${alert.color}`" v-text="alert.message" />
    </transition>
