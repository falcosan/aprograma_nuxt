<template>
  <span :class="`icon-container h-auto grid gap-5 ${blok && blok.tag === 'button' || tag === 'button' ? ' cursor-pointer' : ''}${blok && !blok.remove_space ? ' p-5 md:p-10' : ''}`">
    <component
    ...
    >
      ...
      <svg
        ...
        height="24"
        viewBox="0 0 24 24"
        :class="`back-icon h-auto fill-current my-0 mx-auto ${size}`"
