<template>
  <component
    ...
    :active-class="blok.external_link ? '' : !iconItem && !blok.icon_item ? 'opacity-40 font-semibold' : 'rounded border border-gray-300'"
  ...
  <component
    ...
    v-else
    :class="`item-link h-full cursor-pointer ${sliderMode || carouselMode || containerMode ? 'flex items-center justify-center self-center' : 'block'}`"
    :active-class="active === 'active' ? !iconItem ? 'opacity-40 font-semibold' : 'rounded border border-gray-300' : ''"
    :exact-active-class="active === 'exact' ? !iconItem ? 'opacity-40 font-semibold' : 'rounded border border-gray-300' : ''"
