<template>
  <section :class="`${blok.name.toLowerCase()}-page max-w-sm xs:max-w-md sm:max-w-lg md:max-w-xl lg:max-w-3xl xl:max-w-4xl 2xl:max-w-7xl my-0 mx-auto pb-10 pt-20 px-6`">
    ...
    <div class="container-components grid gap-5 auto-cols-fr" :style="`grid-template-columns:repeat(${maxComponents}, 1fr);`">
      <div
        ...
        :class="`${component.component.toLowerCase()}-content filter drop-shadow-xl ${component.row_container ? '' : 'col-start-1'}`"
      >
        <component
