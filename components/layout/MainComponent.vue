  ...
  <main :class="`main py-10 overflow-x-hidden ${!$device.isDesktop ? '' : 'md:py-14'} ${blok.background_media.filename ? '' : 'bg-white'}`">
    <div :class="`main-wrapper relative overflow-hidden ${$store.state.data.error ? 'main-error' : 'main-regular'}`">
      ...
      <transition enter-active-class="duration-500 in-out" enter-class="opacity-0" mode="out-in">
        <Nuxt :class="`relative max-w-sm xs:max-w-md sm:max-w-lg md:max-w-2xl md:w-9/12 lg:w-auto lg:max-w-3xl xl:max-w-4xl 2xl:max-w-7xl my-0 mx-auto rounded-b-md ${!$device.isDesktop ? '' : 'md:rounded-t-md'}`" />
      </transition>
    </div>
