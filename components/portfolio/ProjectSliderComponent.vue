<template>
  <div class="project-slider relative w-full z-10">
    ...
    >
        ...
        >
          <NuxtLink v-click-outside="focusSlide" :to="`${$route.path}/${project.slug}`" class="project-link w-full grid grid-rows-1 grid-cols-2">
            <div :class="`text-container ${index %2 == 0 ? 'col-start-1 col-end-1 text-right' : 'col-start-2 col-end-2 text-end'} flex flex-col justify-center row-start-1 row-end-1`" :style="`background-color: ${project.content.teaser_background_color.color};`">
              <h2 class="project-text text-xl xl:text-2xl px-5 lg:px-8 overflow-hidden" :style="`color: ${project.content.teaser_text_color.color};`">
      ...
      <li v-if="frame.down === blok.length && blok.length > 2" :key="`${indexControls}-0`" :class="`restart-control control h-full projects-center col-start-1 col-end-1 cursor-pointer bg-opacity-30 bg-white ${blok.length % 2 == 0 ? 'row-start-1 row-end-1 self-end' : 'row-start-2 row-end-2 self-start'}`">
