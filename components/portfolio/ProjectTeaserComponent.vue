<template>
  <li
    v-if="projectContent"
    :class="`project-teaser w-full h-full overflow-hidden rounded ${rowContainer || sliderContainer || carouselContainer || containerContainer ? 'max-h-sd' : !$device.isDesktop ? '' : 'transform transition-all duration-200 hover:shadow hover:scale-105'}`"
  >
