<template>
  <ProjectSlider
    ...
    :blok="sortedProject"
  />
  <ul v-else-if="sortedProject" :class="`project-list grid gap-5 auto-cols-fr ${$route.name === 'portfolio' ? 'auto-rows-fr' : ''} ${blok.row_container ? 'lg:grid-cols-container' : 'md:grid-cols-2 lg:grid-cols-3'}`">
    <ProjectTeaser
