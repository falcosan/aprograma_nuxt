<template>
  <ProjectSlider
    ...
    :blok="sortedProject"
  />
  <ul v-else-if="sortedProject" :class="`project-list grid gap-5 auto-cols-fr ${blok.row_container && $parent.blok.body.filter(item => item.row_container).length > 1 ? 'md:auto-rows-max' : 'md:grid-cols-2 lg:grid-cols-3'}`">
    <ProjectTeaser
